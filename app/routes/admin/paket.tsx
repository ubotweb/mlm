      ...
      <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="xl:col-span-1 bg-[#151921] border border-[#222731] rounded-2xl overflow-hidden shadow-sm h-fit">
          <div class="bg-[#1A1E26] px-6 py-5 border-b border-[#222731]">
            <h4 class="font-black text-white text-sm uppercase tracking-widest">Tambah Paket Baru</h4>
          </div>
          <form method="POST" action="/api/admin/paket" class="p-6 space-y-5">
            <div>
              <label class="block text-[11px] font-black text-[#8B949E] uppercase tracking-widest mb-2">Kode ID Paket</label>
              <input type="text" name="id" required class="w-full bg-[#0B0E14] border border-[#2D3342] text-white rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 text-sm font-mono" placeholder="Contoh: pkg_diamond" />
            </div>
            <div>
              <label class="block text-[11px] font-black text-[#8B949E] uppercase tracking-widest mb-2">Nama Level</label>
              <input type="text" name="name" required class="w-full bg-[#0B0E14] border border-[#2D3342] text-white rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 text-sm" placeholder="Diamond" />
            </div>
            
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-[11px] font-black text-[#8B949E] uppercase tracking-widest mb-2">Jml Hak Usaha (HU)</label>
                <input type="number" name="hu_count" required class="w-full bg-[#0B0E14] border border-[#2D3342] text-emerald-400 font-black rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 text-sm text-center" placeholder="1" />
              </div>
              <div>
                <label class="block text-[11px] font-black text-[#8B949E] uppercase tracking-widest mb-2">Jml Produk</label>
                <input type="number" name="product_count" required class="w-full bg-[#0B0E14] border border-[#2D3342] text-blue-400 font-black rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 text-sm text-center" placeholder="1" />
              </div>
            </div>

            <div>
              <label class="block text-[11px] font-black text-[#8B949E] uppercase tracking-widest mb-2">Biaya Daftar (Rp)</label>
              <input type="number" name="registration_fee" required class="w-full bg-[#0B0E14] border border-[#2D3342] text-white font-bold rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 text-sm tracking-widest" placeholder="150000" />
            </div>
            <div>
              <label class="block text-[11px] font-black text-[#8B949E] uppercase tracking-widest mb-2">Bonus Sponsor (Rp)</label>
              <input type="number" name="sponsor_bonus_amount" required class="w-full bg-[#0B0E14] border border-[#2D3342] text-white font-bold rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 text-sm tracking-widest" placeholder="50000" />
            </div>
            <div>
              <label class="block text-[11px] font-black text-[#8B949E] uppercase tracking-widest mb-2">Diskon Produk RO (%)</label>
              <input type="number" name="discount_percentage" required class="w-full bg-[#0B0E14] border border-[#2D3342] text-white font-bold rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 text-sm" placeholder="10" />
            </div>
            <div class="pt-4 border-t border-[#222731]">
               <label class="flex items-center space-x-3 mb-3 cursor-pointer">
                 <input type="checkbox" name="network_bonus_eligible" value="1" defaultChecked class="w-4 h-4 rounded bg-[#0B0E14] border-[#2D3342]" />
                 <span class="text-sm font-bold text-gray-300">Dapat Bonus Jaringan/Pasangan</span>
               </label>
            </div>
            <button type="submit" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-xl mt-2 transition-colors shadow-lg shadow-blue-600/20 uppercase tracking-widest text-xs">Simpan Paket</button>
          </form>
        </div>
                ...
                {packages.map((p: any) => (
                  <tr class="hover:bg-[#1A1E26] transition-colors">
                    ...
                    <td class="px-5 py-5 text-right">
                      <form method="POST" action="/api/admin/paket/delete" onsubmit="return confirm('Yakin ingin menghapus paket ini?');">
