...
adminPaketApi.use('/*', async (c, next) => {
  ...
  try {
    ...
    // Kembali menggunakan await next() asli Anda yang tidak memicu Promise Error!
    await next()
  } catch (err) {
...
adminPaketApi.post('/create', async (c) => {
  try {
    // PELINDUNG MUTLAK: Seluruh pembacaan dipindah ke dalam try!
    const db = c.env.DB
    ...
    // MEMAKSA TAMPIL DI LOG CLOUDFLARE
    const errMsg = err instanceof Error ? err.message : String(err);
    console.error("\n[API ERROR - CREATE PAKET]", errMsg, "\n");
    return c.redirect(`/admin/paket?error=${encodeURIComponent(errMsg)}`)
  }
})
